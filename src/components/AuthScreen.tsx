...
export default function AuthScreen({ onLoginSuccess, mockUser, onClose }: AuthScreenProps) {
  ...
  // Google Sign-In / Up States
  const [showGoogleModal, setShowGoogleModal] = useState<boolean>(false);
  const [googleIsRegistering, setGoogleIsRegistering] = useState<boolean>(false);
  const [googleNewName, setGoogleNewName] = useState<string>("");
  const [googleNewEmail, setGoogleNewEmail] = useState<string>("");
  const [googleNewDni, setGoogleNewDni] = useState<string>("");
  const [googleError, setGoogleError] = useState<string>("");
  const [showCustomGoogleForm, setShowCustomGoogleForm] = useState<boolean>(false);
  ...
  const googleAccountsList = [
    {
      ...
      dni: mockUser.dni || "70245678",
      university: mockUser.university
    },
    {
      name: "Camila Torres",
      email: "camila.torres.12@unsch.edu.pe",
      avatar: "https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=150&auto=format&fit=crop&q=80",
      dni: "71234567",
      university: "Facultad de Ingeniería de Minas"
    },
    {
      name: "Carlos Mendoza",
      ...
      dni: "72345678",
      university: "Facultad de Ciencias de la Salud"
    }
  ];

  const handleSelectGoogleAccount = (acc: typeof googleAccountsList[0]) => {
    // If it's the mock user, log in as mock user
    if (acc.email.toLowerCase() === mockUser.email.toLowerCase()) {
      onLoginSuccess(mockUser);
      setShowGoogleModal(false);
      return;
    }

    // Check if user is in registered_students
    ...
    if (existing) {
      ...
      // Register them automatically with Google
      const newStudent: UserProfile = {
        firstName: acc.name.split(" ")[0],
        lastName: acc.name.split(" ").slice(1).join(" ") || "Estudiante",
        name: acc.name,
        email: acc.email,
        university: acc.university || "Universidad Nacional de San Cristóbal de Huamanga",
        avatar: acc.avatar,
        balance: 150.0,
        dni: acc.dni,
        isDniVerified: false
      };
      const filtered = savedUsers.filter(u => u.email.toLowerCase() !== acc.email.toLowerCase());
      localStorage.setItem("registered_students", JSON.stringify([...filtered, newStudent]));
  ...
  const handleCustomGoogleSubmit = (e: React.FormEvent) => {
    ...
    if (!googleNewName.trim() || !googleNewEmail.trim()) {
      setGoogleError("Por favor completa los campos obligatorios.");
      return;
    }

    if (!googleNewEmail.includes("@gmail.com") && !googleNewEmail.includes("@unsch.edu.pe") && !googleNewEmail.includes(".edu")) {
      setGoogleError("Por favor ingresa un correo electrónico de Google válido.");
      return;
    }

    const dniRegex = /^\d{8}$/;
    if (!googleNewDni.trim() || !dniRegex.test(googleNewDni.trim())) {
      setGoogleError("Por favor ingresa un número de DNI válido de 8 dígitos.");
      return;
    }

    // Check DNI uniqueness in saved registered users and mockUser
    const savedUsersRaw = localStorage.getItem("registered_students");
    const savedUsers: UserProfile[] = savedUsersRaw ? JSON.parse(savedUsersRaw) : [];
    
    const dniExists = 
      (mockUser.dni === googleNewDni.trim() && mockUser.email.toLowerCase() !== googleNewEmail.trim().toLowerCase()) ||
      savedUsers.some(u => u.dni === googleNewDni.trim() && u.email.toLowerCase() !== googleNewEmail.trim().toLowerCase());

    if (dniExists) {
      setGoogleError("Error: El número de DNI ingresado ya está asociado a otra cuenta registrada.");
      return;
    }

    const firstName = googleNewName.trim().split(" ")[0] || "Estudiante";
    const lastName = googleNewName.trim().split(" ").slice(1).join(" ") || "Google";
    const newStudent: UserProfile = {
      firstName,
      lastName,
      name: googleNewName.trim(),
      email: googleNewEmail.trim().toLowerCase(),
      ...
      balance: 150.0,
      dni: googleNewDni.trim(),
      isDniVerified: false
    };

    const filtered = savedUsers.filter(u => u.email.toLowerCase() !== googleNewEmail.trim().toLowerCase());
    localStorage.setItem("registered_students", JSON.stringify([...filtered, newStudent]));
    onLoginSuccess(newStudent);
    setShowGoogleModal(false);
  };
